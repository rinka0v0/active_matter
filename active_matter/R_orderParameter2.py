...
for n in range(1):
    ...
    for r in range(1,101):
        ...
        print('r:',r)
        order_parameter = 0.0
        # ここではxは密度が一定になるように設定する
        x = np.random.rand(N, 2) * xy_lim 
        ...
        if time == time_limit - 1:
            ...
            if r == 1:
                plt.scatter(IMPACT_RADIUS, order_parameter, c=COLOR[n], label=LABELS[n])
            else:
                plt.scatter(IMPACT_RADIUS, order_parameter, c=COLOR[n])
